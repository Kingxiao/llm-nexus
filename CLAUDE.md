...
- Test (CI-equivalent): `RUSTFLAGS="-D warnings" cargo test --workspace --all-features`
...
- CI 用 `RUSTFLAGS=-D warnings`，本地默认不设。本地 `cargo test --workspace` 通过 ≠ CI 通过。凡是涉及 unused warning（变量、字段、宏、import）的代码，必须用 `RUSTFLAGS="-D warnings" cargo test --workspace --all-features` 在本地验证。集成测试 `common.rs` 会被编译为独立 binary，其中定义的宏/re-export 在该 binary 上下文内属于 unused，需加 `#![allow(unused_macros, unused_imports)]`
